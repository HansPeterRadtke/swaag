...
def test_benchmark_modules_import_without_optional_terminal_bench_dependency() -> None:
    script = """
import builtins
real_import = builtins.__import__
def blocked_import(name, globals=None, locals=None, fromlist=(), level=0):
    if name == "terminal_bench" or name.startswith("terminal_bench."):
        raise ModuleNotFoundError("No module named 'terminal_bench'")
    return real_import(name, globals, locals, fromlist, level)
builtins.__import__ = blocked_import
import swaag.benchmark.terminal_bench_agent
import swaag.benchmark.terminal_bench_local
print("imports-ok")
"""
    env = os.environ.copy()
    env["PYTHONPATH"] = _src_path() + (os.pathsep + env["PYTHONPATH"] if env.get("PYTHONPATH") else "")

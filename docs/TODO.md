...
- [ ] **Replace message-count-triggered history compression with budget-driven context pressure.** **Partial.** `compression.py` currently decides compression from `max_messages_before_compress`. Compression must instead be driven primarily by the actual serialized token budget of the next LLM operation. Message count may be telemetry, never the semantic reason to discard detail.
- [ ] **Remove fixed character sizing as an architectural control for summaries.** **Partial.** Configuration still contains `summary_chars` and benchmark-specific `summary_max_chars`. Summary targets must be expressed from the current token budget for the actual model and operation. Character limits may remain only as defensive UI/storage limits where they are not pretending to model context capacity.
- [ ] **Create one explicit context-compiler abstraction used by every LLM call.** **Partial.** `budgeting.py`, runtime preparation, prompt builders, compression, and token counting already implement pieces. Consolidate them so worker calls, summaries, evaluators, communication calls, research calls, and future semantic reducers all use the same verified compilation path and accounting record.
...
- [ ] **Persist a per-call context accounting/provenance record.** Record context capacity, tokenizer/model, mandatory tokens, output reserve, safety margin, every dynamic component's token cost, final serialized input size, actual output size, projection/source references, and any reduction passes. This must be inspectable in history and benchmark output.
...
- [ ] **Make output reserve operation-specific and semantically appropriate.** **Partial.** `CallBudgetPlan` already has call classes and structured-output floors. Extend this so each semantic operation can request/derive an appropriate output reserve while deterministic code enforces the final number. Benchmark the policy rather than freezing arbitrary ratios.
- [ ] **Eliminate any mismatch between model-reported context capacity and actual request serialization.** Add backend/version-specific verification for llama.cpp and OpenAI-compatible servers, including tool/grammar/chat-template overhead and model switches.
...
- [ ] **Track provenance from every summary/projection back to exact source events.** **Partial.** Summary payloads track source counts; target is exact source ranges/references and projection lineage so lost information can be recovered and compaction quality can be audited.
...
- [ ] **Support re-expansion from raw history after compaction.** Use history-window/search/analyze and archive infrastructure so an LLM can recover exact older facts when a summary is insufficient.
...
- [ ] **Stop injecting the complete enabled-tool registry into every worker context.** **Current mismatch.** README describes the complete enabled-tool registry being included. Implement capability discovery/staged schema loading so tool definitions themselves participate in semantic selection and token budgeting.
- [ ] **Add a compact capability index separate from full tool schemas.** Let an LLM discover relevant capabilities cheaply, then load exact schemas only for selected tools.
...
- [ ] **Store raw tool results durably and context-project them per later call.** **Partial.** History preserves events/results, but prompt assembly must consistently reference raw results and allow verbatim inclusion, targeted retrieval, or LLM reduction based on current context budget.
- [ ] **Give tool-result reducers overarching task context.** Avoid generic isolated summaries that cannot know what matters to the task.
- [ ] **Add explicit token accounting per tool schema and per tool result.** Make these visible in context accounting and benchmarks.

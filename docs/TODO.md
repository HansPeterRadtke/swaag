...
- [ ] **Add an AG-UI adapter for rich streaming UI events.** **Partial.** Cursor-paginated `ag_ui.events` and bounded resumable `ag_ui.subscribe` waits rebuild run/result/failure/cancellation/input-required plus canonical inner tool/status/question/progress events from integrity-linked history references. The bounded wait does not occupy communication inference/control admission. Add an actual AG-UI SSE/WebSocket transport and upstream conformance tests before claiming a complete AG-UI server.
- [ ] **Evaluate/add an A2A adapter for durable external task semantics.** **Partial.** `a2a.send/get/list/cancel/subscribe` project A2A 1.0 messages, blocking versus immediate sends, server-owned task/context IDs, raw attachments, durable states, stable cursor pagination, artifacts, status updates, and initial-task-first bounded subscriptions onto the internal worker API. Context mismatches, unsupported URL fetching, terminal subscriptions, and invalid cancellation states fail explicitly. Add an actual HTTP/JSON-RPC/gRPC streaming binding, Agent Card capability negotiation, authentication, history projection, and upstream SDK conformance before claiming an A2A server.

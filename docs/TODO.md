...
- [ ] **Make overflow recovery iterative and semantic.** **Partial.** Action and independent completion-evaluation calls calculate measured overflow, semantically project the largest reducible raw tool result, rebuild, and re-tokenize for bounded rounds while retaining raw provenance. History analysis also starts with exact events and hierarchically projects oversized reducer inputs without dropping fragments. Extend the same reconstruction contract to summary-prompt overflow and future semantic operations.
- [ ] **Make output reserve operation-specific and semantically appropriate.** **Partial.** The compiler separates per-call operation minimums from per-call desired headroom, preserves full-fidelity input that fits the minimum, and reconstructs action and model-backed capability calls after backend-reported output starvation. Dynamic reducer targets now set desired output directly instead of inheriting a static ratio. Extend adaptive retry to remaining semantic operations and benchmark policy.
...
- [ ] **Support dynamically sized history projections per call.** **Partial.** `history_analyze` compiles exact retrieved events for its specific question and computes projection targets only from that call's measured overflow, preserving exact source hashes. General routing, worker, status, and completion projections still need independent semantic selection and sizing; do not reuse one summary merely because it already exists.

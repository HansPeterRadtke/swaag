...
- Artifacts: `/data/var/swaag/manual-tests/ag-ui-sdk-conformance-repo-20260828T035857/result.json` for durable replay and `/data/var/swaag/manual-tests/ag-ui-sdk-new-run-conformance-20260828T100124Z/result.json` for new-run creation.
- Method: install exact official `@ag-ui/client`, `@ag-ui/core`, and `@ag-ui/encoder` 0.0.59 packages outside the repository. The first probe prepares a completed durable worker and stable external thread/run mapping before replaying it through `HttpAgent`. The second starts with empty durable state and lets `HttpAgent` create a real run against a temporary service whose worker is completed by deterministic lifecycle transitions and whose injected client raises on any model access.
- Result: both runs accepted the real POST shape, decoded `RUN_STARTED`, the complete text-message lifecycle, and successful `RUN_FINISHED`, assembled activity and assistant messages, and returned the exact durable result. The new-run artifact records `inference_allowed=false`, `model_client_accesses=[]`, process exit zero, and empty stderr.
- Scope: this establishes official-client serialization, ordering, replay, new-run creation, and state assembly without model inference. It does not claim official interrupt-resume, client-tool/shared-state, binary transport, capability discovery, or WebSocket conformance.

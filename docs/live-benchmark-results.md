...
The first bounded v4 run at an explicit 8,192-token working window checkpointed all three 10% cases before the server closed the connection at the start of the next case. Early, middle, and late retrieval all passed (837, 835, and 837 exact prompt tokens; 49.036, 46.514, and 50.273 seconds respectively). The resumable artifact is `/data/var/swaag/benchmarks/context-order-2026-08-27-qwen38-27b-window8192-v4.json`; it records 3/9 planned cases complete and must not be reported as a completed matrix. The managed endpoint was healthy again immediately after the disconnect, so the remaining 50% and 90% cases should resume from that checkpoint rather than rerun or erase the evidence.

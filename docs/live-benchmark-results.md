...
This bounded matrix found no positional retrieval failure for its exact-code probe, but it is not evidence that ordering is irrelevant for semantic agent operations. The balanced layout experiment still needs to rotate realistic task, request, history, tool definitions, tool results, and retrieved evidence rather than generalizing from one marker task or one model.

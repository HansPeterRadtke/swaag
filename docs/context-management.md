...
If a summary or projection must fit a target token count, give the semantic operation that concrete per-call target and enough task context to preserve relevant information. Re-tokenize the result and retry semantic reduction if necessary. When even one exact history message cannot fit a summary call, segment it mechanically without dropping bytes, summarize every fragment semantically under measured budgets, and semantically combine those derived fragments. The final history summary still cites the original exact event; intermediate summaries never replace raw history.

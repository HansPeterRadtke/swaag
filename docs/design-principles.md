...
Workspace state, notes, attachment references, and learned prompt instructions enter model context through Layer 2 contributor interfaces rather than bespoke AgentRuntime branches. A disabled capability contributes nothing automatically. The core owns only generic contributor admission, measured budgeting, projection orchestration, and provenance-safe recovery. Contributor-specific selection and authority rules remain with the system capability that owns the data.
...
Model transport failures are classified inside the model subsystem. AgentRuntime consumes only the neutral temporary-service-unavailable predicate and therefore does not depend on HTTP-library exception classes or status-code semantics.
 AgentRuntime also depends on the neutral model-client protocol and factory rather than naming the concrete live client implementation.

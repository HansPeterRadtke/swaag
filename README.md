...
Protocol conformance dependencies remain outside the Python runtime. `scripts/install-mcp-conformance-env.sh` and `scripts/mcp-sdk-conformance.mjs` exercise pinned MCP 2026-07-28 discovery, listing, and tool-call decoding against the real stdio entry point. `scripts/install-a2a-conformance-env.sh` and `scripts/a2a-sdk-conformance.mjs` use the official A2A JavaScript client to resolve Swaag's Agent Card and decode A2A 1.0 list/get/cancel plus subscription streaming. MCP remains a replaceable capability boundary rather than Swaag's task protocol.

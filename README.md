...
The MCP adapter remains a replaceable capability boundary. `scripts/install-mcp-conformance-env.sh` installs the exact official TypeScript SDK under `/data/var/swaag`, and `scripts/mcp-sdk-conformance.mjs` exercises pinned 2026-07-28 discovery, listing, and tool-call decoding against the real stdio entry point without adding Node dependencies to the Swaag runtime.

...
Each :class:`SkillSpec` describes contextual instructions the agent can opt
into. The catalog is intentionally small: ``selection_blurb`` is what the
LLM-driven selector reads to decide relevance, and ``full_instructions`` is
the expanded version that gets injected into the prompt only when the skill
is chosen (progressive disclosure).
